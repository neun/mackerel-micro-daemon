...
- **HTTPS 通信は `openssl s_client` コマンドをパイプで叩く簡易実装です。** TLS 証明書の厳格な検証・タイムアウト制御・リトライ処理は最小限であり、本番環境での利用は自己責任の上でご判断ください。
...
- FreeBSD 14.x 系以上を推奨
  ...
  - 9.x / 10.x は動作確認済み。ただしgit clone等での展開、各スクリプトの修正および openssl 1.0.1系以上の追加インストール等が必須。pkg addはできませんでした。
  - 11.x ～ 12.x は動作未確認のため、自己責任でお試しください。
  - 13.x ～ 15.x は動作確認済み。特に何もせず動くはずです。[releaseにpkg](https://github.com/neun/mackerel-micro-daemon/releases/latest)もあるのでお使いください。
